...
def visualize_strain_field_by_magnitude(
    ...
    흰색→빨강 그라디언트로 시각화하고 원본 비디오와 나란히 배치하여 저장.
    ...
    # 출력 프레임 폭(원본 + 시각화 결과 + 범례 폭)
    out_width = width * 2 + legend_width
    ...
    cap.release()
    out.release()
    print(f"[DONE] '{output_path}' 에 원본 + (흰색→빨강) + 범례가 포함된 시각화 영상이 저장되었습니다.")
...
#     video_name = os.path.splitext(os.path.basename(video_file))[0]
#     output_dir = "/home/beating_contest/vector_size_video_with_original_video"
#     os.makedirs(output_dir, exist_ok=True)
    
#     output_path = os.path.join(output_dir, f"{video_name}_strain_vector_size_with_original_video.avi")
